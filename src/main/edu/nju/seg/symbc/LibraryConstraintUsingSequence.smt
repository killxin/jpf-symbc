...
;LIST
(declare-datatypes (T) ((List (mk-pair (element (Seq T))))))
;Set
(declare-datatypes (T) ((Set (mk-pair (element (Seq T))))))
;TreeSet
(declare-datatypes (T) ((TreeSet (mk-pair (mapping (Array T Bool)) (order (Array T Int))))))
;FILEINPUTSTREAM
(declare-datatypes ((FileInputStream (mk-pair (content (ArrayList Int)) (readPosition Int) (isOpen Bool)))))
;Z3 List: get(0,n,head)
get(i,n,head){
    if(head == null) retrun null;
    else if(i==n) return head;
    else return get(i+1,n,head.next);
}
;START
